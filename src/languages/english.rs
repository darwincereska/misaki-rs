...
impl LanguageRules for English {
    fn apply_rules(&self, word: &str, tag: &str, lexicon: &Lexicon) -> Option<String> {
        ...
        if let Some((ps, _, _)) = lexicon.stem_s(word, tag, None, ctx) {
            Some(ps)
        } else if let Some((ps, _, _)) = lexicon.stem_ed(word, tag, None, ctx) {
            Some(ps)
        } else if let Some((ps, _, _)) = lexicon.stem_ing(word, tag, None, ctx) {
            Some(ps)
        } else {
            None
        }
